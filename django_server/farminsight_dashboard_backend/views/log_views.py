...
from farminsight_dashboard_backend.services import valid_api_key_for_sensor, valid_api_key_for_fpf, write_log_message, \
    ...
    get_log_messages_by_amount, is_admin, is_system_admin, get_organization_by_camera_id
...
def get_log_messages(request, resource_type, resource_id):
    ...
    if resource_type == 'camera':
        ...
        if not is_member(request.user, get_organization_by_fpf_id(resource_id).id):
            return Response(status=status.HTTP_403_FORBIDDEN)
    elif resource_type == 'org':
        if not is_member(request.user, resource_id):
            return Response(status=status.HTTP_403_FORBIDDEN)
